...
class VitonHDDataset(data.Dataset):
    def __init__(self,
        ...
        if phase == 'train':
            ...
            filename = '/kaggle/input/d/mrmody476/high-resolution-viton-zalando-dataset/test_pairs.txt'
            # filename = os.path.join(dataroot, f"{phase}_pairs.txt")
    ...
    def __getitem__(self, index):
        ...
        if "warped_cloth" in self.outputlist:  # Precomputed warped clothing image
            if self.order == 'unpaired':
                person_id = im_name.replace('.jpg', '.png')
                cloth_id = c_name.replace('.jpg', '.png') 
                warped_cloth = Image.open(f'/kaggle/input/testing-fine-tuned-warping-model/sample/flow/upper___{person_id}___{cloth_id}')
